...
<div className='w-[30%] relative aspect-4/6 z-10' style={{ perspective: '3000px' }}>
        ...
        <div className='bg-gray-500 w-[3%] h-full absolute z-11 left-[0%] top-0'></div>
        {dane.map((item, index) => (
         <React.Fragment key={index}>
        {index>0&&(
          <img key={flipCount} src="page.png" style={{width: `${100-(index*2)}%`}} className={`h-[98%] ${isFlipped ? "animate-oc":"animate-noc"} border-b-yellow-500 border-r-2 rounded-r-2xl border-l-amber-50 left-[0%] top-[50%] translate-y-[-50%] absolute object-cover`} alt=""></img>
        )}
        {index===0&&(
          <div key={flipCount} onClick={handleClick} className={`${isFlipped ? 'animate-flip' : 'animate-unflip'} w-full h-full relative z-12 hover:cursor-pointer`} style={{ transformOrigin: 'left center', transformStyle: 'preserve-3d' }}>
