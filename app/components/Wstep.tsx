...
const Wstep = () => {
  ...
  return (
    <div ref={containerRef} id='Wstęp' style={{background: "url('ks.jpg')", backgroundSize: 'cover', backgroundPosition: 'center'}} className='w-full hide h-full opacity-0 animate-pojawianie delay-0 absolute inset-0'>
      ...
      <div style={{backgroundImage: "url('/papyrus1.png')", backgroundSize: 'cover', backgroundPosition: 'center'}} onClick={()=>{n1?setN1(false):setN1(true)}} className='aspect-[0.73] cursor-pointer md:rotate-0 lg:rotate-0 -rotate-90 lg:h-[80%] md:h-[60%] overflow-visible h-[60%] absolute top-[50%] left-[50%] translate-[-50%] animate-papyrus'>
      ...
      <Powrot id="Wstęp" />
      <button className='absolute right-0 bottom-0 p-0 mx-20 my-10 flex scale-70 justify-end'><img onClick={()=>{graj?setGraj(false):setGraj(true)}} className='w-[30%] bg-[rgba(104,87,87,0.3)] rounded-[50%] transition-all duration-300 cursor-pointer hover:-translate-y-1 m-0 p-0' src="button.png" alt="" /></button>
      <audio ref={audioRef} src="/scroll.mp3" preload="auto"></audio>
      <audio id='a1' src="tekst1.mp3"></audio>
      <audio id='a2' src="tekst2.mp3"></audio>
    </div>
  )
}
